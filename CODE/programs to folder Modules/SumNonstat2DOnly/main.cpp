 ...
 *  
 *  			TDEMLineCalc
 *  This file contains code for rename 2D nonstationary task results
 *  
 *  
...
int main(int argc,char **argv)
{
	...
	for(ipls=0;ipls<npls;ipls++)
	{
		...
		sprintf(str,"ball.%d",ipls+1);
		ReadPickets(nall,nTim,fnrm,str,true);
		for(j=0;j<3;j++)
		{
			fsum.clear();
			fsum.resize(nall);
			for(i=0;i<nall;i++)
			{
				...
				fsum[i].CopyPicketInfo(fnrm[i]);
				fsum[i].SetLegend(LegendSummB[j]);
				fsum[i].SummValues2DOnly(fnrm[i].v[j]);
			}
			sprintf(str,"%s.%d",Bname[j],ipls+1);
			WritePickets(0,nall,nTim,fsum,str);
		}

		sprintf(str,"edsall_norm.%d",ipls+1);
		ReadPickets(nall,nTim,fnrm,str,false);
		for(j=0;j<3;j++)
		{
			for(i=0;i<nall;i++)
			{
