/*
 * GENERAL REMARKS
 *  
 *  This code is freely available under the following conditions:
 *  
 *  1) The code is to be used only for non-commercial purposes.
 *  2) No changes and modifications to the code without prior permission of the developer.
 *  3) No forwarding the code to a third party without prior permission of the developer.
 *  
 *  			TDEMLineCalc
 *  This file contains headers for read-write recevires data functions
 *  
 *  
 *  Written by Ph.D. Denis V. Vagin
 *  Novosibirsk State Technical University,
 *  20 Prospekt K. Marksa, Novosibirsk,630073, Russia
 *  vdv_wk@mail.ru
 *  Version 2.0 January 16, 2023
*/

#pragma once

// The class contains the value of the non-stationary signal in the receivers and I/O functions
struct Picket
{
	int iPx,iPy;
	double Px,Py,Pz;
	char leg[256];
	vector<double> t,v[3];
	void Clear();
	void Allocate(int n);
	void AddPicket(Picket &pkt,double cff);
};

void ReadPickets(int nr,int nt,vector<Picket> &EdsAll,char *fname,bool fmem);
void WritePickets(int nr,int nt,vector<Picket> &EdsAll,char *fname);
void WritePickets(int from,int to,int nt,vector<Picket> &EdsAll,char *fname);
void WritePicket(int nr,int nt,vector<Picket> &EdsAll,char *fname,int ind);

void ReadPicketsStat(int nr,vector<PointXYZ> &fld,char *fname);
void WritePicketsStat(int nr,vector<PointXYZ> &fld,char *fname);
void WritePicketsStat(int from,int to,vector<PointXYZ> &fld,char *fname);
