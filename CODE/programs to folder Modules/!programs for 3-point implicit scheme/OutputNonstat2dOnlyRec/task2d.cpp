...
void task2d_Ax::output(int it,vector<float> &enor)
{
	...
	if(tbeg+it>=3)
	{
		_diff_t3(Fx[it],Bx[it],Bx[it-1],Bx[it-2],npntB*NUMBEROFABDIPOLES,times,it);
		_diff_t3(Fy[it],By[it],By[it-1],By[it-2],npntB*NUMBEROFABDIPOLES,times,it);
		_diff_t3(Fz[it],Bz[it],Bz[it-1],Bz[it-2],npntB*NUMBEROFABDIPOLES,times,it);
	}
	else if(tbeg+it==1 || tbeg+it==2)
	{
	...
	if(tbeg+it>=3)
	{
			_diff_t3(H,A[0],A[1],A[2],kpnt*npls,times+tbeg,it);
	}
	else if(tbeg+it==1 || tbeg+it==2)
	{
...
void task2d_Ar::output(int it,vector<float> &enor)
{
	...
	if(tbeg+it>=3)
	{
		_diff_t3(F[it],B[it],B[it-1],B[it-2],2*npntB,times,it);
	}
	else if(tbeg+it==1 || tbeg+it==2)
	{
		...
		for(i=0;i<krect;i++)
		{
			...
			if(!it)
			{
				if(DsFlgNode[rect[i].nodes[2]-1] && DsFlgNode[rect[i].nodes[3]-1])
				{
					for(j=2;j<=3;j++)
					{
						Hphi[4*i+j+4*krect*ipls] += 1.0/(2.0*PI*pnt[rect[i].nodes[j]-1].r);
					}
				}
