...
Time_approx_for_vfem::~Time_approx_for_vfem()
{
	...
	if(En_edges_0) {delete [] En_edges_0; En_edges_0=NULL;}
	if(En_edges_1) {delete [] En_edges_1; En_edges_1=NULL;}
	if(En_edges_2) {delete [] En_edges_2; En_edges_2=NULL;}
...
int Time_approx_for_vfem::Prepare_To_Schema()
{
	...
	if(npntE0 && En_edges_0 == NULL)
	{
		En_edges_0 = new double[Tmap->n*npls]; //     ( -,  -)
		if(En_edges_0 == NULL)
			Memory_allocation_error("En_edges", "Time_approx_for_vfem::Two_Layers_Schema");
	...
	if(npntE0 && En_edges_1 == NULL)
	{
		En_edges_1 = new double[Tmap->n*npls]; //     ( -,  -)
		if(En_edges_1 == NULL)
			Memory_allocation_error("En_edges", "Time_approx_for_vfem::Two_Layers_Schema");
	...
	if(npntE0 && En_edges_2 == NULL)
	{
		En_edges_2 = new double[Tmap->n*npls]; //     ( -,  -)
		if(En_edges_2 == NULL)
			Memory_allocation_error("En_edges", "Time_approx_for_vfem::Two_Layers_Schema");
...
int Time_approx_for_vfem::Two_Layers_Schema(int nlst)
{
	...
	inf.open("e0.edge",ios::binary);
	if(inf)
	{
		cout<<"Reading file "<<"e0.edge"<<endl;
		logfile<<"Reading open file "<<"e0.edge"<<endl;
		a0Exist=true;
	...
	if(!a0Exist)
	{
		...
		Va=new double[d->kuzlov*npls];

		inf.open("v3a.dat",ios::binary);
		if(!inf)
		{
			cout<<"Can't open file "<<"v3a.dat"<<endl;
			logfile<<"Can't open file "<<"v3a.dat"<<endl;
			exit(1);
		}
		inf.read((char *)Va,sizeof(double)*d->kuzlov*npls);
		inf.close();
		inf.clear();
		...
		for(ipls=0;ipls<npls;ipls++)
		{
			for(i=0;i<Tmap->n;i++)
			{
				v1=Tmap->edges[i][0];
				v2=Tmap->edges[i][1];
				for(j=0;j<3;j++){axis[j]=d->xyz[v2][j]-d->xyz[v1][j];}
				len=Norm_Euclid(axis,3);
				etan=-(Va[v2+ipls*d->kuzlov]-Va[v1+ipls*d->kuzlov])/len;
				u_j2[i+ipls*Tmap->n]=u_j1[i+ipls*Tmap->n]=u_j[i+ipls*Tmap->n]=etan*len*0.5;
			}
		}
...

	{
		for(itime=1;itime<3;itime++)
		{
			...
			if(npntE0 && !d->fdirect)
			{
				LoadE0DiffFromRZ(itime,itime-1,itime-2);
			}
					...
					for(l=0;l<m;l++)
					{
						...
						for(j=0; j<12; j++)
						{
							...
							if((itime+1)<ntime)
							{
								...
								{
									value=1.0;
									djdt=((d->currentfunction[itime]-d->currentfunction[itime-1])/dt)*value;
								}
								else
								{
									djdt=dA_dt(time[itime],d->currentfunction[itime-1],d->currentfunction[itime],d->currentfunction[itime+1],
										time[itime+1]-time[itime-1],time[itime+1]-time[itime],time[itime]-time[itime-1],time[itime],time[itime-1],time[itime-2]);
								}
							}
							else
							{
								djdt=(d->currentfunction[itime]-d->currentfunction[itime-1])/dt;
							}
							value=(bout[0]*ls.dx+bout[1]*ls.dy+bout[2]*ls.dz)*(-djdt)*2.0/edlen;
	...
	for(itime=3;itime<_ntime;itime++)//////////////////////////
	{
		...
		if(itime>=tbeg && itime<tend)
		{
			...
			if(!fcalc)
			{
				...
				if(npntE0 && !d->fdirect)
				{
					LoadE0DiffFromRZ(itime,tnum1,tnum2);
				}
						...
						for(l=0;l<m;l++)
						{
							...
							for(j=0; j<12; j++)
							{
								...
								if((itime+1)<ntime)
								{
									...
									{
										value=1.0;
										djdt=((d->currentfunction[itime]-d->currentfunction[itime-1])/(time[itime]-time[itime-1]))*value;
									}
									else
									{
										djdt=dA_dt(time[itime],d->currentfunction[itime-1],d->currentfunction[itime],d->currentfunction[itime+1],
								...
								{
									djdt=(d->currentfunction[itime]-d->currentfunction[itime-1])/(time[itime]-time[itime-1]);
								}
								value=(bout[0]*ls.dx+bout[1]*ls.dy+bout[2]*ls.dz)*(-djdt)*2.0/edlen;
...
int Time_approx_for_vfem::LoadE0FromRZ(int tnum,double *En_edges)
{
	...
	{
		double *Vn=NULL;
		Vn=new double[d->kuzlov*npls];
		inf.open("v3n.dat",ios::binary);
		if(!inf)
		{
			cout<<"Can't open file "<<"v3n.dat"<<endl;
			logfile<<"Can't open file "<<"v3n.dat"<<endl;
			exit(1);
		}
		inf.read((char *)Vn,sizeof(double)*d->kuzlov*npls);
		inf.close();
		inf.clear();
		int v1,v2;
		double len,etan;
		...
		for(ipls=0;ipls<npls;ipls++)
		{
			for(i=0;i<Tmap->n;i++)
			{
				...
				for(j=0;j<3;j++){axis[j]=d->xyz[v2][j]-d->xyz[v1][j];}
				len=Norm_Euclid(axis,3);
				etan=-(Vn[v2+ipls*d->kuzlov]-Vn[v1+ipls*d->kuzlov])/len;
				En_edges[i+ipls*Tmap->n]=etan*len*0.5;
			}
		}
		if(Vn){delete [] Vn; Vn=NULL;}
	}


	return 0;
}
int Time_approx_for_vfem::LoadE0DiffFromRZ(int tnum,int tnum1,int tnum2)
{
	if(tnum<=2)
	{
		// для первого временного слоя
		LoadE0FromRZ(tnum,En_edges_0);
		LoadE0FromRZ(tnum1,En_edges_1);
		int i,j,ipls;
		for(ipls=0;ipls<npls;ipls++)
		{
			for(i=0;i<Tmap->n;i++)
			{
				j=i+ipls*Tmap->n;
				En_edges[j]=-(En_edges_0[j]-En_edges_1[j])/(time[tnum]-time[tnum1]);
			}
		}
	}
	else
	{
		LoadE0FromRZ(tnum,En_edges_0);
		LoadE0FromRZ(tnum1,En_edges_1);
		LoadE0FromRZ(tnum2,En_edges_2);
		_dA_dt_vec(En_edges,En_edges_0,En_edges_1,En_edges_2,time[tnum],time[tnum1],time[tnum2],Tmap->n*npls,time[tnum]);
	}
...
void Time_approx_for_vfem::_dA_dt_vec(double *du_j, double *u_j, double *u_j1, double *u_j2,
								 double t_j, double t_j1, double t_j2,int size, double t)
{
	...
	for(i=0;i<size;i++)
	{
		du_j[i] = -(u_j2[i]*(2.0*t-t_j-t_j1)/(dt1*dt)-u_j1[i]*(2.0*t-t_j-t_j2)/(dt1*dt0)+u_j[i]*(2.0*t-t_j1-t_j2)/(dt*dt0));
	}
}
