 ...
 *  
 *  			TDEMLineCalc
 *  This file contains the code for calculating the eleptic source part of primary field for nonstationary task
 *  
 *  
...
int SolveNonStationarProblemForSpiderWithoutResCalcPoints_2(const char* path)
{
	...
	if(1>=tbeg && 1<tend)
	{
		...
		dt=SMesh.BilinearRZMesh.times[1]-SMesh.BilinearRZMesh.times[0];
		SLAE.A.AddToMatrix(SLAE.n, SLAE.jsize, SLAE.B);
		...
		{
			SLAE.F.v[SMesh.BilinearRZMesh.dFunc.dfNodeA[ipls]+SLAE.n*ipls]+=
					SMesh.BilinearRZMesh.currc[1]*currentDeltaFunc[ipls]/(2*_PI_);
		}
		...
		WriteField2d(SLAE.pU->v,SLAE.n*npls,1);
	...
	if(2>=tbeg && 2<tend)
	{
		...
		dt=SMesh.BilinearRZMesh.times[2]-SMesh.BilinearRZMesh.times[1];
		SLAE.A.AddToMatrix(SLAE.n, SLAE.jsize, SLAE.B);
		SLAE.A.AddToMatrix(SLAE.n, SLAE.jsize, SLAE.C, 1.0/dt);
		...
		{
			SLAE.F.v[SMesh.BilinearRZMesh.dFunc.dfNodeA[ipls]+SLAE.n*ipls]+=
					SMesh.BilinearRZMesh.currc[2]*currentDeltaFunc[ipls]/(2*_PI_);
		}
		...
		WriteField2d(SLAE.pU->v,SLAE.n*npls,2);
	...
	for(indt=3; indt<SMesh.BilinearRZMesh.ntimes; indt++)
	{
		SMesh.fsdiff=false;
		...
		if(indt>=tbeg && indt<tend)
		{
			...
			{
				if((retc=ReadField2d(SLAE.pU1->v,SLAE.n*npls,npre1))!=0)return retc;
				if((retc=ReadField2d(SLAE.pU2->v,SLAE.n*npls,npre2))!=0)return retc;
				SMesh.fsdiff=true;
			}
			...
			if(FirstDecadeStep)
			{
				...
				dt=SMesh.BilinearRZMesh.times[indt]-SMesh.BilinearRZMesh.times[npre2];
				dt1=SMesh.BilinearRZMesh.times[npre1]-SMesh.BilinearRZMesh.times[npre2];
				dt0=SMesh.BilinearRZMesh.times[indt]-SMesh.BilinearRZMesh.times[npre1];
