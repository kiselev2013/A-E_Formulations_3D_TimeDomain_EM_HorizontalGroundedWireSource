...
void Subdomain::AsmGlobalMatrix()
{
	...
	if ((pr = new double[/*n_nodes_c*/n_nodes]) == 0) Memory_allocation_error("pr", "AsmGlobalMatrix");
	if ((x = new double[/*n_nodes_c*/n_nodes]) == 0) Memory_allocation_error("x", "AsmGlobalMatrix");
	...
	logfile<<"n_nodes_c= "<<n_nodes_c<<endl;

	prds.factorize(n_nodes_c,p->ig,p->jg,gg,di,3);
}
