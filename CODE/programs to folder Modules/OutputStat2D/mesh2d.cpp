...
int MeshForVP::ReadRecievers(const char* path, int pntVsize, const PointXYZ* pntV)
{
	...
	boundsVA.beginP=boundsVB.beginP=boundsVA.endP=boundsVB.endP=0;
	boundsVforEfromA.beginP=boundsVforEfromB.beginP=boundsVforEfromA.endP=boundsVforEfromB.endP=0;
	...
	if (meshoptions&moForPED)
	{
		int lqMN;
		vector<PointXYZ> M,N;
		boundsVforMNfromB.beginP=PointList.GetListLength();
		strcpy__m(buf, path);
		inf.open(strcat__m(buf, "\\xyzMN"));
		inf>>lqMN;
		M.resize(lqMN);
		N.resize(lqMN);
		for(i=0;i<lqMN;i++){inf>>M[i]>>N[i];}
		inf.close();
		inf.clear();
		...
		for(j=0;j<NvB;j++)
		{
			...
			for(i=0;i<lqMN;i++)
			{
				...
				if (!PointList.AddToList(PntM)) 
					return RETCODE_NOMEM;
				if (!PointList.AddToList(PntN)) 
					return RETCODE_NOMEM;
				if (!PointList.AddToList(PntO)) 
					return RETCODE_NOMEM;

			}
		}
		boundsVforMNfromB.endP=PointList.GetListLength();
	}
	else
	{
		strcpy__m(buf, path);
