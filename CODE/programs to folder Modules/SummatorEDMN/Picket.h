...
public:
	...
	EdsVal GetValueFromParamola(double t1,double t2,double t3,EdsVal &v1,EdsVal &v2,EdsVal &v3,double t)
	{
		double dt1,dt2,dt3;
		...
		for(k=0;k<Np;k++)
		{
			...
			for(i=0;i<ktime;i++)
			{
				if(pictimes[i]<=time_o[ktime-1])
				{
					...
					if(j>0 && j<(ktime-1))
					{
						tmp=GetValueFromParamola(time_o[j-1],time_o[j],time_o[j+1],piceds[j-1],piceds[j],piceds[j+1],pictimes[i])*Mask[k];
					}
					else if(j<(ktime-2))
					{
						tmp=GetValueFromParamola(time_o[j],time_o[j+1],time_o[j+2],piceds[j],piceds[j+1],piceds[j+2],pictimes[i])*Mask[k];
					}
					else if(j>1)
					{
						tmp=GetValueFromParamola(time_o[j-2],time_o[j-1],time_o[j],piceds[j-2],piceds[j-1],piceds[j],pictimes[i])*Mask[k];
					}
					else if(j<(ktime-1))
					{
						tmp=((piceds[j+1]*(pictimes[i]-time_o[j])+piceds[j]*(time_o[j+1]-pictimes[i]))/(time_o[j]-time_o[j-1]))*Mask[k];
					}
					else if(j>0)
					{
						tmp=((piceds[j]*(pictimes[i]-time_o[j-1])+piceds[j-1]*(time_o[j]-pictimes[i]))/(time_o[j]-time_o[j-1]))*Mask[k];
					}
					else
					{
						tmp=0.0;
