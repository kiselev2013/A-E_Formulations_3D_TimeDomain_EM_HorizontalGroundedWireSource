								...
								double sigz=1)
{
	...
	for (i=0; i<8; i++)
	{
		...
		for (j=0; j<8; j++)
		{
			...
			jy=indY(j);
			jz=indZ(j);
			m[i][j]=(sigx*G1d[ix][jx]*M1d[iy][jy]*M1d[iz][jz]*hx1*hy*hz+
				sigy*M1d[ix][jx]*G1d[iy][jy]*M1d[iz][jz]*hx*hy1*hz+
				sigz*M1d[ix][jx]*M1d[iy][jy]*G1d[iz][jz]*hx*hy*hz1)/36;
		}
	}
	return RETCODE_OK;
}
...
/*!    grad_x(Phi) * (Phi) */
void GradMatrix_X(double Dx[8][8], double hx, double hy, double hz)
{			
	double h1,h2,h3; 
	h1=hy*hz/72; h2=hx*hz/72; h3=hy*hx/72;
...
/*!    grad_y(Phi) * (Phi) */
void GradMatrix_Y(double Dy[8][8], double hx, double hy, double hz)
{			
	double h1,h2,h3; 
	h1=hy*hz/72; h2=hx*hz/72; h3=hy*hx/72;
...
/*!    grad_z(Phi) * (Phi) */
void GradMatrix_Z(double Dz[8][8], double hx, double hy, double hz)
{			
	double h1,h2,h3; 
	h1=hy*hz/72; h2=hx*hz/72; h3=hy*hx/72;
