 ...
 *  
 *  			TDEMLineCalc
 *  This file contains the code for calculation of field A for stationary task
 *  
 *  
...
int SolveStationarProblem3d(const char* path,bool f3d,bool forA0)
{
	...
	if ((retc=ProcessBoundaryConditionsForRect3DForVa(&meshvp3d, &SLAE3D, npls))!=0)
		return retc;
	...
	if(forA0)
	{
		...
		for(ipls=0;ipls<npls;ipls++)
		{
			...
			for(l=0;l<m;l++)
			{
				...
				val=ls.dx/*ls.d_ksi*ls.len*/*ImpStatCur;
		...
		for(ipls=0;ipls<npls;ipls++)
		{
			...
			{
				loc_source &ls=srs[ipls][l];
				val=ls.dy/*ls.d_eta*ls.len*/*ImpStatCur;
				GetRightPartForRect3D(&meshvp3d,&SLAE3D,ls,0,val);
			}
		...
		for(ipls=0;ipls<npls;ipls++)
		{
			...
			{
				loc_source &ls=srs[ipls][l];
				val=ls.dz/*ls.d_dzeta*ls.len*/*ImpStatCur;
				GetRightPartForRect3D(&meshvp3d,&SLAE3D,ls,0,val);
			}
