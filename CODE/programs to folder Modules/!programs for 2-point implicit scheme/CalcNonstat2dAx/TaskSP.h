 ...
 *  
 *  			TDEMLineCalc
 *  This file contains the headers for calculating the eleptic source part of primary field for nonstationary two-layer scheme task
 *  
 *  
...
/*!    */
#define MAXNUMBEROFMATERIALS 40000
...
/*!        */
struct MeshForSpiderWithoutResCalcPoints_2
{
	...
	MeshForSpiderWithoutResCalcPoints_2(const char* path):
		BilinearRZMesh(moForSpider|moDontReadResPoints|moUsePntMtrInfo|moUnloadSolutions)
	{

		int i, j, k, elnumreg, elnum, mespzone;
		string buf;
		ifstream infreg;
		PointRZ p_rz;
		double _hh, _ra, _rb, _za, _zb;

		if ((InitializationRetCode=BilinearRZMesh.Read(path))!=0)
			return;
	}

	~MeshForSpiderWithoutResCalcPoints_2()
	{
	}
