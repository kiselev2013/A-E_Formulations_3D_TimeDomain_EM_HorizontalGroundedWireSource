 ...
 *  
 *  			TDEMLineCalc
 *  This file contains the code for calculating the radial source part of primary field for nonstationary two-layer scheme task
 *  
 *  
		...
		for (indt=1; indt<DMesh.BilinearRZMesh.ntimes; indt++)
		{
			...
			if(indt>=tbeg && indt<tend)
			{
				...
				if(FirstDecadeStep)
				{
					...
					SLAE.A.Clear(SLAE.n, SLAE.jsize);
					dt=DMesh.BilinearRZMesh.times[indt]-DMesh.BilinearRZMesh.times[npre1];
					SLAE.A.AddToMatrix(SLAE.n, SLAE.jsize, SLAE.B);
