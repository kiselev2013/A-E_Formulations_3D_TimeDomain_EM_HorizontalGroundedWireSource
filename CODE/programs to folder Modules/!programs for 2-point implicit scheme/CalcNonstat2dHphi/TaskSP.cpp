 ...
 *  
 *  			TDEMLineCalc
 *  This file contains the code for calculating the grounded radial source part of primary field for nonstationary two-layer scheme task
 *  
 *  
...
int SolveNonStationarProblemForDipole(const char* path,bool _forVEL,bool _forCED)
{
	...
	for (indt=1; indt<DMesh.BilinearRZMesh.ntimes; indt++)
	{
		DMesh.fsdiff=false;
		...
		if(FirstDecadeStep)
		{
			...
			SLAE.A.Clear(SLAE.n, SLAE.jsize);
			dt=DMesh.BilinearRZMesh.times[indt]-DMesh.BilinearRZMesh.times[indt-1];
			SLAE.A.AddToMatrix(SLAE.n, SLAE.jsize, SLAE.B);
		...
		AddDC(&DMesh.BilinearRZMesh,&SLAE,currentDeltaFunc,npls,1.0/dt,indt-1);
