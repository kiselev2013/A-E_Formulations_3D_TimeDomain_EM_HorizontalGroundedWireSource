 ...
 *  
 *  			TDEMLineCalc
 *  This file contains the headers for calculating the grounded radial source part of primary field for nonstationary two-layer scheme task
 *  
 *  
...
/*!    */
#define MAXNUMBEROFMATERIALS 20000
...
struct Mesh
{
	...
	vector<vector<int>> ElementsForCurrent;
	...
	int ReadDc(int npls)
	{
		...
		kt1dc.resize(npls);
		l1dc.resize(npls);
		sprintf(buf,"%s\\kt1dc",taskpath);
		infs.open(buf);
		sprintf(buf,"%s\\l1dc.dat",taskpath);
	...
	void GetElementsForDc(int npls)
	{
		int i,j,k,ipls,in_bc1cat1;
		ElementsForCurrent.resize(npls);
		for(ipls=0;ipls<npls;ipls++)
		{
			ElementsForCurrent[ipls].clear();
			for(k=0;k<krect;k++)
			{
				const Rect& r=rect[k];
				in_bc1cat1=0;
				for(i=2;i<=3;i++)
				{
					for(j=0;j<kt1dc[ipls];j++)
					{
						if(r.nodes[i]==l1dc[ipls][j])
						{
							in_bc1cat1++;
							break;
						}
				...
				if(in_bc1cat1==2)
				{
					ElementsForCurrent[ipls].push_back(k);
					continue;
				}
				in_bc1cat1=0;
				for(i=0;i<=2;i+=2)
...
struct MeshForDipole
{
	...
	MeshForDipole(const char* path,
				bool _forVEL,
				bool _forCED):
	BilinearRZMesh(moForDipole|moUsePntMtrInfo|moUnloadSolutions|(_forVEL?moForVEL:0)|(_forCED?moForCED:0))
	{
	...
	void SwapMuSigma()
	{
		for (int i=0; i<MAXNUMBEROFMATERIALS; i++)
			swap(BilinearRZMesh.mu[i], BilinearRZMesh.sigma[i]);
	}
};
