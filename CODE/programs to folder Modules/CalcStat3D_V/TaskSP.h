 ...
 *  
 *  			TDEMLineCalc
 *  This file contains the headers for calculation of secondary part of field V for stationary task
 *  
 *  
...
class Mesh3DForVP: public AbstractFEM3D
{
public:
	...
	int Read(const char* path, bool read_times=false)
	{
		...
		strcpy__m(buf, path);
		inf.open(strcat__m(buf, "\\xyz0.dat"), ios::binary);
		if(inf)
		{
			pnt0=new PointXYZ[kpnt];
			for(i=0;i<kpnt;i++){inf > pnt0[i];}
			inf.close();
		}
		inf.clear();
		...
		strcpy__m(buf, path);
		if ((retc=ReadResCalcFile(strcat__m(buf, "\\xyzvectorErec"), 
								PointList, 
								boundsVforE, 
								gResCalcPoint1, 
								gResCalcPoint2, 
	...
	/*! -   */
	virtual int GetElementNodesNumber()
	{
		return 8;
	}

	/*!     */
	virtual const pv::Point3D GetNode(const int& i_node)
	{
		return pv::Point3D(pnt[i_node].x, pnt[i_node].y, pnt[i_node].z);
	}

	/*!     */
	virtual const pv::Point3D GetNodeTrue(const int& i_node)
	{
		return pv::Point3D(pntt[i_node].x, pntt[i_node].y, pntt[i_node].z);
	}

	/*!     */
	virtual int GetNodeNumberOnElement(const int& i_element, const int& i_node)
	{
		return rect3d[i_element].nodes[i_node]-1;
	}

	/*!    */
	virtual int GetElementMaterial(const int& i_element)
	{
		return rect3d[i_element].mtr-1;
	}

	/*!   */
	virtual int GetTypeOfElement(const int& i_element)
	{
		return rect3d[i_element].rtype-1;
	}

	/*!      */
	virtual double GetValueInElementCenter(const int& i_element, const Res3DValueType& r_type)
	{
		
		int i;
		
		const Rect3D& r3d=rect3d[i_element];
		double x[8], y[8], z[8], q1[8], q2[8];
		double grad1[3], grad2[3];

		for (i=0; i<8; i++)
		{
			const PointXYZ& p=pnt[r3d.nodes[i]-1];
			x[i] = p.x;
			y[i] = p.y;
			z[i] = p.z;
		}
		...
		switch (r_type)
		{
			...
			case vtRotzA: 
				...
				{
					q1[i]=q_res1[r3d.nodes[i]-1];
					q2[i]=q_res2[r3d.nodes[i]-1];
				}
				L.GetGradInHexCenter(q1, grad1, 1);
				L.GetGradInHexCenter(q2, grad2, 0);
				break;
		}
		switch (r_type)
		{
			case vtU:
				return L.GetValueInHexCenter(q1);
			case vtRotxA: 
				return grad2[1]-grad1[2];
			case vtRotyA:
				return grad1[2]-grad2[0];
			case vtRotzA:
				return grad2[0]-grad1[1];
		}
		return 0;
	}

	/*!      */
	virtual void GetValueInElementNodes(const int& i_element, const Res3DValueType& r_type,double *ves)
	{
		...
		switch (r_type)
		{
			case vtU:
				for (i=0; i<8; i++)
					ves[i]=q_res1[r3d.nodes[i]-1];
				break;
		}
	}

	/*! -    */
	virtual int GetNumberOfResPoints(const Res3DValueType& r_type)
	{
		switch (r_type)
		{
			case vtWithDiscontinuity:
				return boundsV.endP-boundsV.beginP;
			case vtWithoutDiscontinuity: 
				return boundsB.endP-boundsB.beginP;
		}
		return 0;
	...
	/*!    */
	virtual pv::Point3D GetResPoint(const Res3DValueType& r_type, const int& i_point)
	{
		switch (r_type)
		{
			case vtWithDiscontinuity:
			{
				const PointForResultCalculationInMesh3D& cpnt=ResCalcPointArray[boundsV.beginP+i_point];
				return pv::Point3D(cpnt.p.x, cpnt.p.y, cpnt.p.z);
			}
			case vtWithoutDiscontinuity:
			{
				const PointForResultCalculationInMesh3D& cpnt=ResCalcPointArray[boundsB.beginP+i_point];
				return pv::Point3D(cpnt.p.x, cpnt.p.y, cpnt.p.z);
			}
		}
		return pv::Point3D();
	...
	/*!    regular */
	virtual int * GetPointerToRegular()
	{
		return &(regular[0]);
	}
	...
	{
		return n_mesh_regular_x;
	}

	/*!   Y   */
	virtual int GetYSize()
	{
		return n_mesh_regular_y;
	}

	/*!   Z   */
	virtual int GetZSize()
	{
		return n_mesh_regular_z;
	}

	/*!  X   */
	virtual double *GetPointerToX()
	{
		return mesh_regular_x;
	}

	/*!  Y   */
	virtual double *GetPointerToY()
	{
		return mesh_regular_y;
	}

	/*!  Z   */
	virtual double *GetPointerToZ()
	{
		return mesh_regular_z;
	}

	/*!   */
	virtual void SaveResult(const Res3DValueType& r_type, const double& r_value, const int& i_point, const int& i_time)
	{
		switch (r_type)
		{
			case vtU:
			{
				PointForResultCalculationInMesh3D& cpnt=ResCalcPointArray[boundsV.beginP+i_point];
				cpnt.res[i_time].x=r_value;
				break;
			}
			case vtRotxA:
			{
				PointForResultCalculationInMesh3D& cpnt=ResCalcPointArray[boundsB.beginP+i_point];
				cpnt.res[i_time].x=r_value;
				break;
			}
			case vtRotyA:
			{
				PointForResultCalculationInMesh3D& cpnt=ResCalcPointArray[boundsB.beginP+i_point];
				cpnt.res[i_time].y=r_value;
				break;
			}
			case vtRotzA:
			{
				PointForResultCalculationInMesh3D& cpnt=ResCalcPointArray[boundsB.beginP+i_point];
				cpnt.res[i_time].z=r_value;
				break;
			}
		}
	}
